...
<div style="max-width: 600px; margin: 0 auto;">
    <h1 style="margin-bottom: 1rem; color: #c9a227;">Edit Trading Cycle</h1>
    ...
    <div class="card">
        <form method="POST" action="{{ route('admin.cycles.update', $cycle) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control" type="text" id="name" name="name" value="{{ $cycle->name }}" required>
                @error('name')<span class="form-feedback-error">{{ $message }}</span>@enderror
            </div>

            <div class="form-group">
                <label for="slug">Slug</label>
                <input class="form-control" type="text" id="slug" name="slug" value="{{ $cycle->slug }}" required>
                @error('slug')<span class="form-feedback-error">{{ $message }}</span>@enderror
            </div>

            <div class="form-group">
                <label for="description">Description</label>
                <textarea class="form-control" id="description" name="description" rows="4">{{ $cycle->description }}</textarea>
                @error('description')<span class="form-feedback-error">{{ $message }}</span>@enderror
            </div>

            <div class="form-group">
                <label for="duration_days">Duration (Days)</label>
                <input class="form-control" type="number" id="duration_days" name="duration_days" value="{{ $cycle->duration_days }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control" type="number" id="daily_profit_percent" name="daily_profit_percent" step="0.01" value="{{ $cycle->daily_profit_percent }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control" type="number" id="total_return_percent" name="total_return_percent" step="0.01" value="{{ $cycle->total_return_percent }}" required>
