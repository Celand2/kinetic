...
@section('title', __('transactions.deposit_title'))
...
<h1 style="color:#c9a227; font-size:1.2rem; margin-bottom:1.25rem;">{{ __('transactions.deposit_request') }}</h1>

@if($paymentMethods->count())
<div class="card" style="margin-bottom:1.25rem; padding:1rem 1.25rem;">
    <div style="font-size:0.72rem; text-transform:uppercase; letter-spacing:0.08em; color:#6b7a9a; margin-bottom:0.75rem; font-weight:600;">{{ __('transactions.available_payment_methods') }}</div>
    ...
    <form method="POST" action="{{ route('transactions.deposit.store') }}" enctype="multipart/form-data" id="depositForm">
        ...
        <div id="depositStep1" style="display:{{ $initialStep === 1 ? 'block' : 'none' }};">
            <div class="form-group">
                <label class="form-label" for="payment_method">{{ __('transactions.choose_payment_method') }}</label>
                @if($lockedMethod)
            ...
            <div id="paymentDetails" style="display:none; margin-top:-0.75rem; margin-bottom:1rem; background:rgba(107,122,154,0.07); border:1px solid rgba(107,122,154,0.2); border-radius:8px; padding:0.65rem 1rem;">
                <div style="font-size:0.78rem; color:#b0bfd9; margin-bottom:0.4rem;">{{ __('transactions.payment_details') }}</div>
                ...
                <div style="margin-top:0.5rem; color:#c9a227; font-size:0.82rem;">{{ __('transactions.keep_screenshot') }}</div>
            </div>

            <div style="display:flex; justify-content:flex-end; gap:0.75rem; margin-top:1rem;">
                <button type="button" class="btn btn-secondary" onclick="goToStep(2)">{{ __('transactions.next') }}</button>
            </div>
        </div>

        <div id="depositStep2" style="display:{{ $initialStep === 2 ? 'block' : 'none' }};">
            <div class="form-group">
                <label class="form-label" for="payment_method">{{ __('transactions.selected_payment_method') }}</label>
            ...
            <div class="form-group">
             ...
             <div id="conversionPreview" style="display:none; margin-top:-0.75rem; margin-bottom:1rem; background:rgba(201,162,39,0.07); border:1px solid rgba(201,162,39,0.2); border-radius:8px; padding:0.65rem 1rem;">
                <div style="font-size:0.78rem; color:#b0bfd9;">{{ __('transactions.usd_equivalent') }}</div>
            ...
            <div class="form-group">
                <label class="form-label" for="screenshot">{{ __('transactions.payment_screenshot') }}</label>
                <input type="file" id="screenshot" name="screenshot" class="form-control" accept="image/*" required>
                <span class="form-hint">{{ __('transactions.screenshot_hint') }}</span>
            ...
            <div class="form-group">
                ...
                <textarea id="description" name="description" class="form-control" rows="3" placeholder="{{ __('transactions.notes_placeholder') }}">{{ old('description') }}</textarea>
            </div>

            <div style="display:flex; justify-content:space-between; gap:0.75rem; margin-top:1rem; flex-wrap:wrap;">
                <button type="button" class="btn btn-secondary" onclick="goToStep(1)">← {{ __('common.back') }}</button>
                <button type="submit" class="btn btn-primary" style="flex:1; min-width:160px;">{{ __('transactions.submit_request') }}</button>
            </div>
...
function goToStep(step) {
    if (step === 2) {
        ...
        if (!methodSelect.value) {
            alert(@json(__('transactions.choose_method_alert')));
            return;
        }
    }
    showStep(step);
    ...
    if (localAmt > 0 && currency) {
        ...
        if (currency !== 'USD') {
            document.getElementById('rateInfo').textContent =
                @json(__('transactions.rate_used')) + rate.toLocaleString('fr-FR', {maximumFractionDigits: 2}) + ' ' + currency;
        } else {
            document.getElementById('rateInfo').textContent = '';
        }
